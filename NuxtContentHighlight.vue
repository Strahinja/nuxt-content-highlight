...
{
    const { highlightText, highlightClassName } = options;
    if (highlightText.length>0 && node)
    {
        if (node.text && node.text.length >= highlightText.length)
        {
            ...
            while (i < node.text.length)
            {
                ...
                if (compareText.toLowerCase()==highlightText.toLowerCase())
                {
...
export default {
    ...
    render (h, { data, props })
    {
        const { document, highlightText, highlightClassName } = props;
        const { body } = document || {};
